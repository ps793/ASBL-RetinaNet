...
class backbone(nn.Module):
    def __init__(self, num_classes=1000):
         ...
         self.features= nn.Sequential(*list(model_vgg.children())[0])             
         self.conv6 = nn.Conv2d( 512, 256, kernel_size=3, stride=2, padding=1) #512
...
class FocalLoss(nn.Module):
	...
	#self.features= nn.Sequential(*list(model_res.children())[0:6])
        ...
        self.c3= nn.Sequential(*list(c_res.children())[0:24]).eval()
        self.c4= nn.Sequential(*list(c_res.children())[24:34]).eval()
        self.c5= nn.Sequential(*list(c_res.children())[34:44]).eval()
